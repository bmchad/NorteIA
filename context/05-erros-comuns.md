...
| ⭐⭐ `git push` publica a Edge Function | ⛔ **Não publica, e medido em 2026-09-24.** Não existe `.github/workflows/`, e o commit `90e5411` voltou `total_count: 0` em `/check-runs` — a integração nativa do Supabase sempre deixa um check no commit. Ela estava ligada ao repositório **antigo** e não seguiu para o novo, criado em 2026-09-25T00:20Z. A Vercel publica no push; a Edge Function, **nunca** | este arquivo |
| O front e a Edge Function sobem juntos | ⛔ Corolário do acima, e é a armadilha cara: toda mudança de contrato entre os dois é par indivisível com deploy **assimétrico** — o front chega primeiro, sempre. Ponte de compatibilidade, nunca janela | este arquivo |
